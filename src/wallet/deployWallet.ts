...
export default async function deployHighload() {
    const result = await compileFunc({
        targets: ['highload_wallet.fc'], // targets of your project
        sources: {
            'stdlib.fc': fs.readFileSync('./src/wallet/stdlib.fc', { encoding: 'utf-8' }),
            'highload_wallet.fc': fs.readFileSync('./src/wallet/highload_wallet.fc', {
                encoding: 'utf-8'
            })
        }
    });
    ...
    const walletMnemonicArray = process.env.MAIN_WALLET_MNEMONIC!.split(' ');
    const walletKeyPair = await mnemonicToWalletKey(walletMnemonicArray); // extract private and public keys from mnemonic
    const walletAddress = Address.parse(process.env.MAIN_WALLET_ADDRESS!);
    ...
    const toSign = beginCell()
        .storeUint(/*user_id % 2 ** 32*/ 698983191, 32) // subwallet_id
        .storeUint(Math.floor(Date.now() / 1e3) + 60, 32) // Transaction expiration time, +60 = 1 minute
        .storeUint(seqno, 32) // store seqno
        .storeUint(0, 8) // Do not forget that if we use Wallet V4, we need to add .storeUint(0, 8)
        .storeUint(3, 8)
    ...
    const external = beginCell()
        ...
        .storeCoins(toNano(0.1)) // Import fee
    ...
    console.log(`===> New High-Load wallet depolyed,\n===> CA: ${contractAddress}`);

    return { CA: contractAddress.toString(), mnemonic: highloadMnemonicArray.join(' ') };
}
